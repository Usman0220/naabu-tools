...
impl IpGenerator {
    ...
    pub fn reserved_ranges_display() -> Vec<(String, String)> {
        reserved_ranges()
            .into_iter()
            .map(|r| (format!("{} - {}", r.start, r.end), String::new()))
            .collect()
    }
}
...
mod tests {
    ...
    fn test_cidr_expansion() {
        let ips = IpGenerator::from_cidr("192.168.1.0/30").unwrap();
        assert_eq!(ips.len(), 0); // private, all excluded
        let ips = IpGenerator::parse_ip_list("8.8.8.0/30");
        assert_eq!(ips.len(), 2);
    }
}